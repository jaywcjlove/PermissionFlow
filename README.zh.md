...
[English](./README.md) • [繁體中文](./README.zh-tw.md) • [安装](#安装) • [本地化](#本地化) • [对外 API](#对外-api) • [System Settings URL Scheme](#system-settings-url-scheme)
...
宿主界面要显示辅助功能隐私页名称时，用 `accessibilityNameResource`。它是绑在包资源 bundle 上的 `LocalizedStringResource`，SwiftUI 会自动跟随**主应用语言**。macOS 27 起系统设置将该页标为 **设备控制和数据访问**（Device Control and Data Access）；更早版本仍是 **辅助功能**（Accessibility）。设置 URL 和 `Privacy_Accessibility` 锚点不变。

```swift
import PermissionFlow
import SwiftUI

// SwiftUI：跟随主应用语言（主应用中文 → 中文名称）
Text(PermissionFlowResources.accessibilityNameResource)
```

AppKit 里用 `String(localized:)` 收成 `String`：
...
如果要自己查字符串，`accessibilityName()` 会返回对应的本地化 key。把任意 NameKey 传给 `localizedString(for:)` 即可从包内 `.lproj` 取出译文，或传给 `localizedStringResource(for:)` 给 SwiftUI 用：
